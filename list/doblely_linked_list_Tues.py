...
class DLinkedList:                       
    ...
    def insert(self, pos, elem):
        ...
        if pos < 0: return
        new = DNode(elem) # 1. 새 노드 삽입
        before = self.getNode(pos-1) # 2. pos-1 위치의 노드 탑색
        if before == None: # 1) 머리노드로 삽입
            if pos == 0:
                new.next = self.head
                if new.next is not None:
                    new.next.prev = new
        ...
        if before == None:
            if pos == 0: # 머리노드로 삭제
                ...
                if self.head is not None:
                    ...
                    self.head.prev = None # 새로운 머리노드의 이전 노드는 None
