...
3. append(node): 현재 노드(self) 뒤에 주어진 노드(new)를 연결
...
class LinkedList:
    ...
    def getNode(self, pos):
        # pos번째에 있는 노드를 반환하기
        # 리스트의 인덱스는 0부터 시작
        if pos < 0: return None # pos는 invalid 경우
    ...
    def insert(self, pos, elem):
        ...
        if pos < 0 : return # 자동으로 유효하지않는 위치에 대래 ValueError

        new = Node(elem) # 또는 Node(elem, Node) # 1. 새 노드 생성
    ...
    def replace(self, pos, elem):
        # 리스트의 pos에 있는 노드의 데이터 변경(QUIZ. 106)
